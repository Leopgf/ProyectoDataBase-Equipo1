...
class EditarPromosAdmin extends Component {
    ...
    componentDidMount() {
      ...
      axios.get(`http://localhost:8000/api/promociones/${this.props.match.params.id_promocion}`).then((res) => {
        const promociones = res.data[0];
    ...
    handleEditar() {
        const {
            id, nombre, descripcion, fecha_inicio, fecha_fin, descuento, estado
        } = this.state.promociones

        axios
        .put(`http://localhost:8000/api/promociones/${this.state.promociones[0].id}/`, {
          nombre,
          descripcion,
          fecha_inicio, 
          fecha_fin,
